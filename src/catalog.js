...
export const catalog = [
  ...
  op('WSLegislativo', 'retornarProyectoLey', '/proyectos/:boletin', 'single', [p('boletin', 'prmNumeroBoletin', 'string')], 600, { db: { repo: 'proyectos', metodo: 'proyectoPorBoletin', param: 'prmNumeroBoletin' } }),
  ...
  op('WSLegislativo', 'retornarVotacionesXAnno', '/votaciones', 'collection', [q('anno', 'prmAnno', 'int')], 300, { legible: true, db: { repo: 'votaciones', metodo: 'votacionesPorAnno', param: 'prmAnno' } }),
  op('WSLegislativo', 'retornarVotacionesXProyectoLey', '/votaciones', 'collection', [q('boletin', 'prmNumeroBoletin', 'string')], 300, { legible: true }),
  op('WSLegislativo', 'retornarVotacionDetalle', '/votaciones/:id', 'single', [p('id', 'prmVotacionId', 'int')], 600, { legible: true, votosPerfil: true, db: { repo: 'votaciones', metodo: 'votacionDetalle', param: 'prmVotacionId' } }),

  op('WSDiputado', 'retornarDiputados', '/diputados', 'collection', [], 600),
  op('WSDiputado', 'retornarDiputadosPeriodoActual', '/diputados/periodo-actual', 'collection', [], 600, { flatten: 'diputado', enrich: 'distrito', db: { repo: 'diputados', metodo: 'periodoActual' } }),
  ...
  op('WSDiputado', 'retornarDiputado', '/diputados/:id', 'single', [p('id', 'prmDiputadoId', 'int')], 600, { db: { repo: 'diputados', metodo: 'porId', param: 'prmDiputadoId' } }),
  ...
  op('WSSala', 'retornarSesionesXAnno', '/sesiones', 'collection', [q('anno', 'prmAnno', 'int')], 300, { db: { repo: 'sesiones', metodo: 'sesionesPorAnno', param: 'prmAnno' } }),
  op('WSSala', 'retornarSesionesXLegislatura', '/sesiones', 'collection', [q('legislatura', 'prmLegislaturaId', 'int')]),
  op('WSSala', 'retornarSesionAsistencia', '/sesiones/:id', 'single', [p('id', 'prmSesionId', 'int')], 600, { db: { repo: 'sesiones', metodo: 'sesionDetalle', param: 'prmSesionId' } }),
